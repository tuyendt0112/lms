...
import logo2 from 'assets/logohcmute2.png'
...
const Header = () => {
    ...
    return (
        <div className='w-main flex justify-between h-[110px] py-[35px] mb-2'>
            <Link to={`/${path.HOME}`}>
                <img src={logo2} alt="logo" className='w-[340px] object-contain ' />
            </Link>
            <div className='flex text-[13px]'>
                {current &&
                    ...
                    >
                        ...
                        {isShowOption && <div onClick={e => e.stopPropagation()} className='absolute top-full flex-col flex left-[10px] bg-gray-100 border min-w-[150px] py-2 rounded-md'>
                            <Link className='p-2 w-full hover:bg-sky-100' to={`/${path.MEMBER}/${path.PERSONAL}`}
                            >Personal
                            </Link>
                            {+current.role === 1 && <Link className='p-2 w-full hover:bg-sky-100' to={`/${path.ADMIN}/${path.MANAGE_STUDENT}`}
                            >Admin Workspace
                            </Link>}
