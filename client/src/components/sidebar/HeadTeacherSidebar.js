...
import logo from 'assets/logohcmute2.png'
import { adminSideBar, headTeacherSideBar } from 'ultils/constant'
...
const HeadTeacherSidebar = ({ open, setOpen }) => {
    ...
    return (
        <div className={`bg-dark-purple overflow-y-auto h-full py-4 ${open ? 'w-72' : 'w-20'} duration-700`}>
            <div>
                ...
                <h1 className={`text-white font-bold text-center duration-500 ${!open && "scale-0"}`}>Head Teacher Workspace</h1>
            </div>
            <BsArrowLeftShort
...
export default memo(HeadTeacherSidebar)