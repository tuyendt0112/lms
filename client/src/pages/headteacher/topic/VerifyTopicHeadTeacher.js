...
import { apiGetTopics, apiDeleteTopic, apiUpdateTopic } from "apis"
...
const VerifyTopicHeadTeacher = () => {
    ...
    const handleAcceptTopic = async (data) => {
        console.log(data)
        const finalPayload = {
            status: "Validated"
        }
        ...
        window.scrollTo(0, 0);
        const response = await apiUpdateTopic(finalPayload, data?._id);
    ...
    const handleRejectTopic = async (data) => {
        console.log(data)
        const finalPayload = {
            status: "Pending"
        }
                    ...
                    {topics?.map((el, index) => (
                        ...
                        >
                            ...
                            <td className="text-center">
                                {el.status === 'Pending' ?
                                    <div className="border-dashed border-2 border-yellow-500 rounded-lg py-2">
                                        <span className="text-yellow-500">
                                            {el.status}
                                        </span>
                                    </div>
                                    :
                                    <div className="border-dashed border-2 border-green-500 rounded-lg py-2">
                                    ...
                                    {el.status === 'Pending' ?
                                        <>
                                            <button
                                                ...
                                                onClick={() => handleAcceptTopic(el)}
                                            >
                                                Accept
                                            </button>
                                            <button
                                                onClick={() => handleRejectTopic(el)}
...
export default VerifyTopicHeadTeacher