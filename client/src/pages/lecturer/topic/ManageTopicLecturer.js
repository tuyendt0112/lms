...
import UpdateTopicLecturer from "./UpdateTopicLecturer";
import DetailTopicLecturer from "./DetailTopicLecturer";
import { useSelector } from "react-redux";
const { AiFillStar, MdEdit, MdDeleteForever, FaArrowRight } = icons;

const ManageTopicLecturer = () => {
    ...
    const fetchTopics = async (params) => {
        const response = await apiGetTopics({
            ...params,
            instructors: current?._id,
            major: current?.marjor,
            ...
            <table className="table-auto w-full ">
                <thead className="text-md text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr className="bg-sky-900 text-white  py-2">
                        ...
                        <th className="px-4 py-2 text-center h-[60px] w-[100px] ">Reviewer</th>
                        <th className="px-4 py-2 text-center h-[60px] w-[100px] ">
                            Date Start
                        </th>
                        <th className="px-4 py-2 text-center h-[60px] w-[100px] ">
                ...
                <tbody>
                    {topics?.map((el, index) => (
                        ...
                        >
                            ...
                            <td className="text-center py-2">
                                <div className="flex flex-col items-center justify-center">
                                    <sm >{el?.reviewer?.firstname} {el?.reviewer?.lastname} </sm>
                                </div>
                            </td>
                            <td className="text-center py-2">
...
export default ManageTopicLecturer