...
const createUser = asyncHandler(async (req, res) => {
  const { email, password, firstname, lastname, role, major, department } = req.body
...
const getLecturer = asyncHandler(async (req, res) => {
  ...
  const formartedQueries = JSON.parse(queryString)
  formartedQueries["$or"] = [
    { role: { $regex: 2, $options: "i" } },
    { role: { $regex: 3, $options: "i" } },
  ]
  // Filtering
  // regex: tìm từ bắt đầu bằng chữ truyền vào
  // options: 'i' không phân biệt viết hoa viết thường
...
const updateUsers = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  const { firstname, lastname, email, codeId, major, department, phoneNumber } = req.body;
  const data = { firstname, lastname, email, codeId, major, department, phoneNumber };
